...
### Changed
- **Breaking**: `CloneFeature` now clones exactly one feature — `features` must hold a single key, where it previously accepted any number. A serialized `CloneFeature` naming two or more features no longer loads; replace it with one `CloneFeature` per feature. The `OneFeatureKeys` type alias, which only this field used, is removed in favour of an inline annotation.
