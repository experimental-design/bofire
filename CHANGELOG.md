...
### Added
- `MixedKernel`: a continuous and a categorical kernel combined as in the mixed GP of Wan et al. (2021), usable in any single-task GP. The categorical kernel works on the categorical inputs encoded as ordinal codes, the continuous kernel on everything else, engineered features included; either can be restricted with `features`. Model components now request the encoding they need, so a `SingleTaskGPSurrogate` with a `MixedKernel`, or with `ICMKernel` and `TaskConstantMean`, builds without setting `categorical_encodings`: categoricals without an explicit encoding get ordinal codes where a component asks for them. An encoding set explicitly always wins and is rejected if the component cannot work with it.
- `ICMKernel` for multi-task models: a base kernel multiplied by a learned correlation between tasks, over the domain's `CategoricalTaskInput`. `TaskConstantMean` and `TaskGaussianLikelihood` learn one constant and one noise variance per task. The number of tasks is read from the task input, not configured.
- `AdditiveMapSaasKernel`: a sum of sparse (SAAS-prior) Matern kernels at different sparsity levels, usable in any single-task GP and inside `AdditiveKernel`, `MultiplicativeKernel`, `ScaleKernel` and `PolynomialFeatureInteractionKernel`, e.g. restricted to descriptor features next to a categorical kernel.
- The mean function and the likelihood of a Gaussian process are data models of their own, like kernels: `ConstantMean` (optionally with a prior and lower and upper bounds on the constant) in `bofire.data_models.means.api`, and `GaussianLikelihood` in `bofire.data_models.likelihoods.api`. `SingleTaskGPSurrogate` takes them as `mean` and `likelihood`. This is the first step of #825.
...
- SHAP feature importance supports categorical outputs by explaining every category probability, including multiclass and mixed continuous/categorical output predictors.
...
### Changed
- `MixedSingleTaskGPSurrogate` is fitted as a `SingleTaskGP` with a `MixedKernel`; predictions are identical and the data model is unchanged. The continuous and categorical kernels passed in are no longer modified, so one kernel can be reused across surrogates (#820). Engineered features now reach the continuous kernel; before, the mixed GP silently ignored them. `features` set explicitly on either kernel are honoured instead of having to match the automatic split, e.g. a continuous kernel may also work on ordinal codes. The messages for a mixed GP without a categorical input change to "MixedKernel needs at least one categorical input encoded as ordinal codes.". Step 5 of #825.
- `MultiTaskGPSurrogate` is fitted as a `SingleTaskGP` built from `ICMKernel`, `TaskConstantMean` and `TaskGaussianLikelihood` -- the model BoTorch's `MultiTaskGP` builds by default, with identical predictions. Its data model is unchanged. Two things change for fitted models: the noise is now learned per task, as BoTorch intends, instead of shared across tasks (a regression introduced in #766); and a task without observations is predicted from the prior instead of raising. The `UserWarning` for an unused `task_prior` is now emitted when the surrogate is built rather than fitted. With several multi-task outputs, the combined model is a `ModelListGP` instead of a generic `ModelList`. The functional class `bofire.surrogates.api.MultiTaskGPSurrogate` is removed; build it through `surrogates.map` as before. Dumps of models fitted earlier still load. Step 4 of #825.
- **Breaking**: `SingleTaskGPSurrogate` no longer has `noise_prior` and `noise_constraint`; both live on its new `likelihood`. Serialized surrogates carrying the flat keys no longer load. Migrate `SingleTaskGPSurrogate(noise_prior=p, noise_constraint=c)` to `SingleTaskGPSurrogate(likelihood=GaussianLikelihood(noise_prior=p, noise_constraint=c))`. The other surrogates keep their flat noise fields for now.
- `AdditiveMapSaasSingleTaskGPSurrogate` is fitted as a `SingleTaskGP` built from `AdditiveMapSaasKernel`, a constant mean with a `Normal(0, 1)` prior bounded to [-10, 10], and the default Gaussian likelihood. This is the model BoTorch's `AdditiveMapSaasSingleTaskGP` builds, and predictions are identical; only the class of the fitted model changes. Dumps of models fitted earlier still load. The functional class `bofire.surrogates.api.AdditiveMapSaasSingleTaskGPSurrogate` is removed; build it through `surrogates.map` as before. Step 3 of #825.
- The default noise model of `SingleTaskGPSurrogate` is BoTorch's: a log-normal prior on the noise variance with a lower bound of 1e-4, as before, but the fit now starts at the prior's mode instead of GPyTorch's generic initial value. Fitted models can therefore differ slightly from earlier releases.
- **Breaking**: kernels, means and likelihoods are checked against the features they are applied to when the surrogate is built, instead of failing at fit time or fitting a meaningless model. Rejected are a feature key that names nothing, a categorical kernel on a continuous input or engineered feature, an `IndexKernel`/`PositiveIndexKernel` not applied to exactly one ordinal-encoded categorical with `num_categories` categories, a fidelity kernel on anything but a `ContinuousTaskInput`, and a conditional kernel whose conditions name unknown features. Kernels inside `ScaleKernel`, `AdditiveKernel` and the other composites are checked too. Combinations that are merely unusual stay allowed, e.g. an RBF kernel over ordinal codes. Applies to `SingleTaskGPSurrogate`, `RobustSingleTaskGPSurrogate`, `TanimotoGPSurrogate`, `PairwiseGPSurrogate`, `LinearSurrogate` and `PolynomialSurrogate`; step 2 of #825.
- **Breaking**: `SingleTaskIBNNSurrogate` is **removed**. It only fixed the kernel of a single-task GP; use `SingleTaskGPSurrogate(kernel=InfiniteWidthBNNKernel(), hyperconfig=None)`. Serialized surrogates carrying `{"type": "SingleTaskIBNNSurrogate"}` no longer load.
