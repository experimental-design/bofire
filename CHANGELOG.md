...
### Changed
- **Breaking**: `SingleTaskIBNNSurrogate` is **removed**. It only narrowed `SingleTaskGPSurrogate`'s kernel and mapped to its functional class, and the kernel it fixed is a one-liner to set by hand. Serialized surrogates carrying `{"type": "SingleTaskIBNNSurrogate"}` no longer load; build `SingleTaskGPSurrogate(inputs=i, outputs=o, kernel=InfiniteWidthBNNKernel(), hyperconfig=None)` instead. `InfiniteWidthBNNKernel` itself is unchanged and still in `AnyKernel`.
- `LinearSurrogate` and `PolynomialSurrogate` subclass `SingleTaskGPSurrogate` rather than `TrainableBotorchSurrogate`, and live in `bofire.data_models.surrogates.presets` together. Their field set, defaults and serialized form are unchanged, so this is not breaking; `isinstance(surrogate, SingleTaskGPSurrogate)` is now true, and `hyperconfig` narrows from `Hyperconfig` to `SingleTaskGPHyperconfig`.
