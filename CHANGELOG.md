...
### Added
- The mean function and the likelihood of a Gaussian process are data models of their own, like kernels: `ConstantMean` (optionally with a prior and a constraint on the constant) in `bofire.data_models.means.api`, and `GaussianLikelihood` in `bofire.data_models.likelihoods.api`. `SingleTaskGPSurrogate` takes them as `mean` and `likelihood`. This is the first step of #825.
