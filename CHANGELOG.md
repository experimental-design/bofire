...
- Descriptor and structure data is described to the LLM again. The four removed feature classes each overrode the `to_pydantic_field()` description hooks; deleting them dropped the descriptor values (`descriptors per category: {...}` for categoricals, `descriptors: {...}` for numericals) and the molecular marker (`... molecular (SMILES: ...)`) from the schema built by `Inputs.to_pydantic_model()`, leaving a materially weaker prompt. Descriptions now match the previous output and additionally list a `structure` block that sits beside human-readable categories — a case the removed classes could not express. `DiscreteInput`, which gained `descriptors` in this release, is covered too.
