...
### Fixed
- `PositiveIndexKernel` accepts the last category as `target_task_index`. The validator rejected `num_categories - 1`, one short of the bound the underlying botorch kernel enforces, so a three-category kernel could only be normalized against categories 0 and 1.
