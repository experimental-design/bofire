...
from bofire.surrogates.mlp import MLP, MLPDataset, fit_mlp
...
class _MLPClassifierEnsemble(EnsembleModel):
    ...
    def forward(self, X: Tensor):
        r"""Assumes that the OUTPUT of the MLPs are the logits and hence we take the softmax over the last dimension here
        ...
        """
        return torch.stack(
            [nn.functional.softmax(mlp(X), dim=-1) for mlp in self.mlps], dim=-3
        )
...
# ):
#     """Fit a MLP for classification to a dataset.

#     Args:
#         mlp (MLP): The MLP that should be fitted.
#         dataset (ClassificationDataSet): The data that should be fitted
