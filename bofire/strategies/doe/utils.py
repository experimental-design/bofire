...
#     NChooseKConstraint,
# )
# from bofire.data_models.features.api import ContinuousInput, Input, ContinuousOutput
...
#     constraints=[
#         NonlinearEqualityConstraint(expression="x1**2 + x2**2 + x3**2 - 1", features=["x1","x2","x3"], jacobian_expression="[2*x1,2*x2,2*x3]"),
#         NChooseKConstraint(features=["x1","x2","x3","x4"], min_count=1, max_count=3, none_also_valid=False),
#     ]
# )


# constraints = get_nchoosek_constraints(domain)

# # x = pd.DataFrame([[0,0,0,0],[1,2,3,4],[0,1,2,3],[0,-1,0,1],[2,0,0,0]], columns=["x1","x2","x3","x4"])
# # x = torch.tensor(x.values, dtype=torch.double, device="cpu", requires_grad=True)

# n = 200


# x = torch.randn(size=(n,n), requires_grad=True)


# def f(x):
#     return (torch.logdet(x.T@x + 1e-4**(1/n) * torch.eye(n)))

# f(x).backward()
# x.grad
# # %%
# X = x.detach().T@x.detach() + 1e-4**(1/n) * torch.eye(n)
# X = np.array(X)
# times = []
# for i in range(100):
#     t = time()
