...
class ExactPiecewiseLinearGPSurrogateHyperconfig(Hyperconfig):
    type: Literal["ExactPiecewiseLinearGPSurrogateHyperconfig"] = (
        "ExactPiecewiseLinearGPSurrogateHyperconfig"
    )
    inputs: Inputs = Inputs(
        features=[
            CategoricalInput(
...
class ExactPiecewiseLinearGPSurrogate(TrainableBotorchSurrogate):
    """GP surrogate that is based on a `WassersteinKernel` for modeling functions
    that take a monotonically increasing piecewise linear function as input. The
    computation of the covariance between the piecewise linears is done by the
    Wasserstein distance kernel. The continuous features are modeled by a separate
    ...
    type: Literal["ExactPiecewiseLinearGPSurrogate"] = "ExactPiecewiseLinearGPSurrogate"  # type: ignore
    interpolation_range: Bounds
    n_interpolation_points: PositiveInt = 1000
    x_keys: list[str]
    y_keys: list[str]
    ...
    append_y: Annotated[List[float], AfterValidator(validate_monotonically_increasing)]
    normalize_y: PositiveFloat = 1.0
    hyperconfig: Optional[ExactPiecewiseLinearGPSurrogateHyperconfig] = Field(  # type: ignore
        default_factory=lambda: ExactPiecewiseLinearGPSurrogateHyperconfig(),
    )

    shape_kernel: Union[ExactWassersteinKernel, WassersteinKernel] = Field(
        default_factory=lambda: ExactWassersteinKernel(
            squared=False,
            lengthscale_prior=LogNormalPrior(loc=1.0, scale=2.0),
        ),
    )
