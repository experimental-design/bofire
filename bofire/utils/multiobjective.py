...
def get_ref_point_mask(domain:Domain, output_feature_keys:Optional[list]=None)->np.array:
    if output_feature_keys is None:
        output_feature_keys = domain.output_features.get_keys_by_objective(excludes=None)
    ...
    for key in output_feature_keys:
        ...
        if isinstance(feat.objective,MaximizeObjective):
            ...
            raise ValueError("Only `MaximizeObjective` and `MinimizeObjective` supported.")
...
def compute_hypervolume(domain:Domain, optimal_experiments:pd.DataFrame, ref_point: dict) -> float:
    ...
    hv = Hypervolume(
        ref_point=torch.from_numpy(np.array([ref_point[feat] for feat in domain.output_features.get_keys_by_objective(excludes=None)])*ref_point_mask))
    return hv.compute(torch.from_numpy(optimal_experiments[domain.output_features.get_keys_by_objective(excludes=None)].values*ref_point_mask))
...
def infer_ref_point(domain:Domain, experiments:pd.DataFrame,return_masked:bool = False):
    ...
    ref_point_array = (df[domain.output_features.get_keys_by_objective(excludes=None)].values*mask).min(axis=0)
    if return_masked == False:
        ref_point_array/=mask
    return {feat: ref_point_array[i] for i, feat in enumerate(domain.output_features.get_keys_by_objective(excludes=None))}
