...
class InterpolateTransform(InputTransform, Module):
    """Botorch input transform that interpolates values between given x and y values on a specified new_x."""
...
class TrapezoidTransform(InputTransform, Module):
    """Botorch input transform that interpolates values at the set of unique x values in X (after prepending, appending, and normalization) and then performs trapezoidal integration."""
    ...
    def transform(self, X: Tensor):
        ...
        # Compute unique, sorted x values from all rows (flattened)
        # unique_x = torch.unique(x.reshape(-1), sorted=True)
        flat_x = x.reshape(-1)
        # sort flat_x
        flat_x = torch.sort(flat_x).values

        # print('Unique X for integration:', unique_x)

        # expand and interpolate
        new_x = flat_x.expand(x.shape[0], -1)

        print("shape of x", x.shape, "shape of flattened x", x.reshape(-1).shape)
        new_y = torch.vmap(interp1d)(x, y, flat_x.expand(x.shape[0], -1))

        print("new_y", new_y.shape)

        # print('Interpolated Y for integration:', new_y)

        # Trapezoidal integration
        # dx = new_x[:, 1:] - new_x[:, :-1]
        # avg_y = 0.5 * (new_y[:, 1:] + new_y[:, :-1])
        # new_y = torch.sum(dx * avg_y, dim=-1, keepdim=True)

        # Reshape new_y to match batch dimensions
        if X.dim() == 3:
            new_y = new_y.reshape((shapeX[0], shapeX[1], new_y.shape[-1]))
            new_x = new_x.reshape((shapeX[0], shapeX[1], new_x.shape[-1]))
        if X.dim() == 4:
            new_y = new_y.reshape((shapeX[0], shapeX[1], shapeX[2], new_y.shape[-1]))
            new_x = new_x.reshape((shapeX[0], shapeX[1], shapeX[2], new_x.shape[-1]))
