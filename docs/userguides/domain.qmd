...
- `DiscreteInput`
- `CategoricalInput`
- `CategoricalInput` carrying descriptor data
- `TaskInput`
...
<!-- #### Continuous descriptor inputs
 -->
#### Categorical inputs with descriptors
Via a `CategoricalInput`'s `descriptors` table one can provide continuous encodings for the different categories via so called descriptors. Imagine, for example, having a categorial input with different categories, where each category corresponds to a specific material. Such descriptors could be e.g., density and hardness. Every material/category would get assigned a number for density and hardness in the hope that these two properties describe the material properly. In the context of fitting a GP, one can then use just these two dimensional vector for describing the material instead of a ten dimensional one-hot encoding, which results in a dimensionality reduction. Of course, this makes sense under the assumption that the descriptors actually correlate with the desired quantities.

`descriptors` is a table with one column per descriptor and one value per category, in the
order of `categories`. For example, the following code defines a new categorical input
variable $x_6$ with values "material_A", "material_B", "material_C".

```{python}
from bofire.data_models.features.api import CategoricalInput, Descriptors

CategoricalInput(
    key="x6",
    categories=["material_A", "material_B", "material_C"],
    descriptors=Descriptors(
        columns={"density": [0.1, 0.3, 0.5], "hardness": [0.2, 0.4, 0.6]}
    ),
)

```

#### Categorical inputs with molecular structures
A categorical input can carry a `structure` column of valid SMILES strings, which
descriptor generators (`Fingerprints`, `Fragments`, `MordredDescriptors`) turn into
model columns. The following code defines a new categorical input variable $x_7$ where
...
CategoricalInput(
    key="x7",
    categories=["C1CCCCC1", "O1CCOCC1"],
    descriptors=Descriptors(structure=["C1CCCCC1", "O1CCOCC1"]),
)
```
...
inputs = Inputs(
    ...
    DiscreteInput(key="x3", values=[0, 0.1, 0.2]),
    CategoricalInput(key="x4", categories=["A", "B", "C"]),
    CategoricalInput(key="x6", categories=["material_A", "material_B", "material_C"], descriptors=Descriptors(columns={"density": [0.1, 0.3, 0.5], "hardness": [0.2, 0.4, 0.6]})),
    CategoricalInput(key="x7", categories=["C1CCCCC1", "O1CCOCC1"], descriptors=Descriptors(structure=["C1CCCCC1", "O1CCOCC1"]))])
```
