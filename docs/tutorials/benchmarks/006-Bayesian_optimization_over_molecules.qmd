...
smiles = list(set(main_file[benchmark["input"]].to_list()))
input_feature = CategoricalInput(
    key=benchmark["input"],
    categories=smiles,
    # the categories *are* SMILES, so they double as the structure column
    descriptors=Descriptors(structure=smiles),
)
...
for _ in range(n_iter):
    ...
    specs = {
        Benchmark.domain.inputs.get_keys()[0]: DescriptorEncoding(
            columns=[],
            generators={"smiles": [Fingerprints(n_bits=2048), Fragments()]},
        )
    }
