...
from everest.domain.tests.test_features import (
    ...
    VALID_CONTINUOUS_OUTPUT_FEATURE_SPEC, VALID_FIXED_CATEGORICAL_INPUT_FEATURE_SPEC,
...
def generate_experiments(
        ...
        [
            {
                ...
                **{
                    f.key: random.choice(f.categories) if not only_allowed_categories else random.choice(f.get_allowed_categories())
    ...
    if force_all_categories:
        for feat in domain.get_features(CategoricalInputFeature):
            categories = feat.categories if len(feat.categories) <= row_count else feat.categories[:row_count]
...
def generate_invalid_candidates_bounds(domain, row_count: int = 5, error = "lower"):
    ...
    if error == "lower":
        candidates.loc[0, domain.get_feature_keys(ContinuousInputFeature)[0]] = domain.get_features(ContinuousInputFeature)[0].lower_bound-0.1
    else:
        candidates.loc[0, domain.get_feature_keys(ContinuousInputFeature)[0]] = domain.get_features(ContinuousInputFeature)[0].upper_bound+0.1
    return candidates
...
@pytest.mark.parametrize("domain, experiments, strict", [
...
@pytest.mark.parametrize("domain, candidates", [
    (d, generate_candidates(d))
    for d in domains
])
...
@pytest.mark.parametrize("domain, candidates, cols", [
...
@pytest.mark.parametrize("domain, candidates, key", [
