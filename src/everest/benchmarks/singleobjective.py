...
from everest.domain.desirability_functions import (
    MaxIdentityDesirabilityFunction, MinIdentityDesirabilityFunction)
from everest.domain.features import (CategoricalDescriptorInputFeature,
                                     ...
                                     OutputFeature)
from everest.study import Study
...
class SingleObjective(Study):
    ...
    def __init__(self,**data):
        super().__init__(**data)
        if len(self.domain.get_features(OutputFeature, excludes=[ContinuousOutputFeature_woDesFunc])) > 1: #TODO: update, when more features without DesFunc are implemented!
    ...
    def get_fbest(self, experiments: Optional[pd.DataFrame]=None):
        ...
        ofeat = self.domain.get_features(OutputFeature, excludes=[ContinuousOutputFeature_woDesFunc])[0] 
...
class Himmelblau(SingleObjective):
    ...
    def setup_domain(self):
        ...
        domain.add_feature(ContinuousInputFeature(key="x_2",lower_bound=-4., upper_bound=6.)) # ToDo, check for correct bounds
...
class Hartmann3D(SingleObjective):
    ...
    def setup_domain(self):
        ...
        # Input features
        feature1 = ContinuousInputFeature(key="x_1", lower_bound=0, upper_bound=1)
        feature2 = ContinuousInputFeature(key="x_2", lower_bound=0, upper_bound=1)
        feature3 = ContinuousInputFeature(key="x_3", lower_bound=0, upper_bound=1)
        
        # target
        output1 = ContinuousOutputFeature(key="y", desirability_function = MaxIdentityDesirabilityFunction(w=1))
...
class Ackley(SingleObjective):
    ...
    def setup_domain(self):
        ...
        # Decision variables
        if self.categorical:
            domain.add_feature(CategoricalInputFeature(key='category', categories=[str(x) for x in range(self.num_categories)]))
        
        if self.descriptor:
            domain.add_feature(CategoricalDescriptorInputFeature(key='descriptor', categories=[str(x) for x in range(self.num_categories)], descriptors = ['d1'], values=[ [x*2] for x in range(self.num_categories)]))

        # continuous input features
        for d in range(self.dim):
            domain.add_feature(ContinuousInputFeature(key=f"x_{d+1}", lower_bound = self.lower, upper_bound=self.upper))

        # Objective
        domain.add_feature(ContinuousOutputFeature(key="y", desirability_function = MaxIdentityDesirabilityFunction(w=1)))
        return domain
