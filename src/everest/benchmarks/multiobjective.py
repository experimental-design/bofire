...
class MultiObjective(Study):
    ...
    def __init__(self, **data):
        super().__init__(**data)
        if len(self.domain.get_features(OutputFeature, excludes=[ContinuousOutputFeature_woDesFunc])) < 2: #TODO: update, when more features without DesFunc are implemented!
            raise ValueError("received singelobjective domain.")

class DTLZ2(MultiObjective):

    num_objectives: PositiveInt = 2
    dim: PositiveInt
    k: Optional[int]
    ...
    def setup_domain(self):
        ...
        for i in range(self.dim):
            domain.add_feature(ContinuousInputFeature(key="x_%i" % (i),lower_bound=0.0,upper_bound=1.0))
        self.k = self.dim - self.num_objectives + 1
        for i in range(self.num_objectives):
            domain.add_feature(ContinuousOutputFeature(key=f"f_{i}",desirability_function=MinIdentityDesirabilityFunction(w=1.)))
        self.ref_point = {feat:1.1 for feat in domain.get_feature_keys(ContinuousOutputFeature)}
        return domain
    ...
    def run_candidate_experiments(self, candidates):
        ...
        for i in range(self.num_objectives):
            ...
            fs.append(f_i)
        candidates[self.domain.get_feature_keys(OutputFeature, excludes=[ContinuousOutputFeature_woDesFunc])]=np.stack(fs,axis=-1)
        candidates[["valid_%s" % feat for feat in self.domain.get_feature_keys(OutputFeature, excludes=[ContinuousOutputFeature_woDesFunc])]] = 1
...
class SnarBenchmark(MultiObjective):
    ...
    def setup_domain(self):
        ...
        domain.add_feature(ContinuousInputFeature(key="tau", lower_bound=0.5, upper_bound=2.))

        # "equivalents of pyrrolidine"
        domain.add_feature(ContinuousInputFeature(key="equiv_pldn", lower_bound=1.0, upper_bound=5.))

        # "concentration of 2,4 dinitrofluorobenenze at reactor inlet (after mixing) in M"
        domain.add_feature(ContinuousInputFeature(key="conc_dfnb", lower_bound=0.1, upper_bound=0.5))

        # "Reactor temperature in degress celsius"
        domain.add_feature(ContinuousInputFeature(key="temperature", lower_bound=30, upper_bound=120.))

        # Objectives
        # "space time yield (kg/m^3/h)"
        domain.add_feature(ContinuousOutputFeature(
            key="sty",
