...
from everest.benchmarks.singleobjective import Ackley
...
from everest.strategies.botorch.tests.test_qehvi import (
    VALID_BOTORCH_QEHVI_STRATEGY_SPEC,
)
from everest.strategies.botorch.tests.test_sobo import VALID_BOTORCH_SOBO_STRATEGY_SPEC
...
def test_ask_multi_objective(
    ...
    # set up of the strategy
    strategy = cls(**spec, ref_point=benchmark.ref_point if use_ref_point else None)
    strategy.init_domain(benchmark.domain)
    strategy.tell(experiments)
