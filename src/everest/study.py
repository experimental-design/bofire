...
from everest.domain.features import InputFeature, OutputFeature, ContinuousOutputFeature, ContinuousOutputFeature_woDesFunc
...
class PoolStudy(BaseModel):
    ...
    def validate_metrics(cls, metrics, values):
        if metrics is MetricsEnum.HYPERVOLUME and len(values["domain"].get_features(ContinuousOutputFeature,excludes=ContinuousOutputFeature_woDesFunc)) < 2:
    ...
    def validate_ref_point(cls,ref_point, values):
        ...
        if len(ref_point) != len(values["domain"].get_features(ContinuousOutputFeature,excludes=ContinuousOutputFeature_woDesFunc)):
            raise ValueError("Length of refpoint does not match number of output features.")
        for feat in values["domain"].get_feature_keys(ContinuousOutputFeature,excludes=ContinuousOutputFeature_woDesFunc):
    ...
    def optimize(
        ...
        with tqdm(range(num_iterations),disable=True if progress_bar == False else False, postfix={"dist2best":"?"}) as pbar:
            for i in pbar:
                ...
                acqf_values = strategy.calc_acquisition(self.open_experiments)
