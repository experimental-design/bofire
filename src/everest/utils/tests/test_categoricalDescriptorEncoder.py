...
X_expected_ignore = np.array([[1., 2.],[0., 0.],[3.,7.]])

data_unknownCat = pd.DataFrame.from_dict({
    "c1": ["A","U","B"]
})

enc = CategoricalDescriptorEncoder(**VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT)

@pytest.mark.parametrize("data, data_unknownCat, expected", [
    (data, data_unknownCat, X_expected_ignore)
])
def test_categorical_descriptor_encoder_handle_unknown(
    data, data_unknownCat, expected
):
    
    # Test that one hot encoder raises error for unknown features
    # present during transform.
    enc = CategoricalDescriptorEncoder(**VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT, handle_unknown="error")
    enc.fit(data)
    with pytest.raises(ValueError, match="Found unknown categories"):
        enc.transform(data_unknownCat)

    # Test the ignore option, ignores unknown features (giving all 0's)
    enc = CategoricalDescriptorEncoder(**VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT, handle_unknown="ignore")
    enc.fit(data)
    ...
    enc = CategoricalDescriptorEncoder(**VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT, handle_unknown="42")
...
def test_categorical_descriptor_encoder_transform(output_dtype):
    ...
    enc = CategoricalDescriptorEncoder(**VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT, dtype=output_dtype)
...
@pytest.mark.parametrize("data, data_unknownCat, X_expected", [(data, data_unknownCat, X_expected)])

def test_categorical_descriptor_encoder_categories(data, data_unknownCat, X_expected):
    
    # auto-generated categories
    enc = CategoricalDescriptorEncoder(categories="auto", values = VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT["values"])
    enc.fit(data)
    ...
    enc = CategoricalDescriptorEncoder(categories=cats, values = VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT["values"])
    ...
    with pytest.raises(ValueError, match="Found unknown categories"):
     ...
     # Test the ignore option, ignores unknown features (giving all 0's)
    enc = CategoricalDescriptorEncoder(categories=cats, values = VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT["values"], handle_unknown="ignore")
    
    assert_array_equal(enc.fit(data_unknownCat).transform(data_unknownCat), X_expected_ignore)
...
def test_categorical_descriptor_encoder_descriptors(data,  X_expected):
    ...
    enc = CategoricalDescriptorEncoder(descriptors = 'auto', values = VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT["values"])
    enc.fit(data)
    ...
    enc = CategoricalDescriptorEncoder(descriptors = des, values = VALID_CATEGORICAL_DESCRIPTOR_ENCODER_INPUT["values"])
