...
@pytest.mark.parametrize("domain, descriptor_encoding, categorical_encoding", [(
    domain, 
...
def test_transformer_init(domain, descriptor_encoding, categorical_encoding):
    ...
    feature_names = [transformer.features2transformedFeatures.get(feat, [feat]) for feat in domain.get_feature_keys()]
    feature_names = [names for sublist in feature_names for names in sublist]

    if descriptor_encoding == DescriptorEncodingEnum.DESCRIPTOR or categorical_encoding == CategoricalEncodingEnum.ONE_HOT:

        feature_dict = transformer.features2transformedFeatures
        feature_names_from_init = [feature_name for list in feature_dict.values() for feature_name in list]
        ...
        assert len(set(feature_names_from_init).intersection(set(feature_names_from_transform)))>0
...
@pytest.mark.parametrize("domain, categorical_encoding, descriptor_encoding, scale_inputs, scale_outputs, data", [
...
def test_transformer_transform(
    ...
    transformer = Transformer(domain=domain, descriptor_encoding = descriptor_encoding, categorical_encoding = categorical_encoding,\
...
@pytest.mark.parametrize("domain, scale_inputs", [
    (domain, scale_inputs)
    for scale_inputs in [i.name for i in ScalerEnum]
    ])
...
@pytest.mark.parametrize("domain, descriptor_encoding, categorical_encoding, exp", [
...
def test_transformer_get_features_to_be_transformed(
    ...
    transformer = Transformer(domain=domain, categorical_encoding=categorical_encoding, descriptor_encoding=descriptor_encoding)
    features = transformer.get_features_to_be_transformed()

    assert len(features) == exp

@pytest.mark.parametrize("domain", [domain])
def test_transformer_not_initialized(
    domain: Domain
):  
...
@pytest.mark.parametrize("domain, df_data, descriptor_encoding, categorical_encoding, exp_dtype", [
    ...
    for descriptor_encoding, categorical_encoding, exp_dtype in [
        ("DESCRIPTOR", "ONE_HOT", [np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64')]),
        ("DESCRIPTOR", "ORDINAL", [np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('int32'), np.dtype('float64')]),
        ("DESCRIPTOR", None, [np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('O'), np.dtype('float64')]),
        ("CATEGORICAL", "ONE_HOT", [np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64')]),
        ("CATEGORICAL", "ORDINAL", [np.dtype('float64'), np.dtype('int32'), np.dtype('int32'), np.dtype('float64')]),
        ("CATEGORICAL", None, [np.dtype('float64'), np.dtype('O'), np.dtype('O'), np.dtype('float64')]),
        (None, "ONE_HOT", [np.dtype('float64'), np.dtype('O'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64'), np.dtype('float64')]),
        (None, "ORDINAL", [np.dtype('float64'), np.dtype('O'), np.dtype('int32'), np.dtype('float64')]),
        (None, None, [np.dtype('float64'), np.dtype('O'), np.dtype('O'), np.dtype('float64')])
    ]
    ])
