...
def calc_rhofact(solvent_type, Tfact):
    x = solvent_descriptors['density'][solvent_type] #  between 0.7 and 1.1
    x = (1.5-x) * (Tfact+0.5)/2
    return x.values
...
def create_experiments(domain, nsamples=100, A=25, B=90, candidates=None):
    ...
    if candidates is None:
        ...
        solvent_types = [
            domain.input_features.get_by_key('Solvent Type').categories[np.random.randint(0,3)] for i in range(nsamples)
        ]
    else:
