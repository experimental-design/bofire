...
if __name__ == "__main__":
    ...
    surrogate_data = dm_surrogates.TanimotoGPSurrogate(
        inputs=input_features,
        outputs=output_features,
        ...
        input_preprocessing_specs={
            # "molecule": MolecularEncodingEnum.FINGERPRINTS(n_bits=32, bond_radius=3),
            "molecule": dm_molfeatures.Fingerprints(n_bits=32, bond_radius=3),
            # 'molecule': MolecularEncodingEnum.FINGERPRINTS(),
            # 'molecule': MolecularEncodingEnum.FRAGMENTS(),
            # 'molecule': MolecularEncodingEnum.FRAGMENTS(fragments=['fr_unbrch_alkane', 'fr_thiocyan']),
            # 'molecule': MolecularEncodingEnum.FINGERPRINTS_FRAGMENTS(n_bits=64, bond_radius=3),
            # 'molecule': MolecularEncodingEnum.FINGERPRINTS_FRAGMENTS(n_bits=64, bond_radius=3, fragments=['fr_unbrch_alkane', 'fr_thiocyan']),
            # "molecule": MolecularEncodingEnum.MORDRED_DESCRIPTORS(descriptors=["NssCH2", "ATSC2d"]),
            # "molecule": dm_molfeatures.MordredDescriptors(descriptors=["NssCH2", "ATSC2d"]),
     },
    )
    ...
    jspecafter = surrogate_data.json()
    ...
    surrogate_data = parse_obj_as(dm_surrogates.AnySurrogate, json.loads(jspecafter))
    surrogate3 = surrogates_api.map(surrogate_data)
    surrogate3.loads(dump)

    # predict with it
    df_predictions2 = surrogate2.predict(experiments) # re-generates descriptors
    df_predictions3 = surrogate3.predict(experiments) # descriptors pre-loaded
    # transform to spec
    predictions2 = surrogate2.to_predictions(predictions=df_predictions2)
    predictions3 = surrogate3.to_predictions(predictions=df_predictions3)

    # check for equality
    assert predictions == predictions2 == predictions3
