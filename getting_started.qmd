...
Any input can additionally carry a `Descriptors` block, which describes what its values *are*:
- numeric `columns` give one value per category (or a single value, for a continuous or discrete input), so a model can reason about how the options differ rather than treating them as unrelated labels.
- a `structure` column holds SMILES, which surrogates can turn into molecular featurizations (`Fingerprints`, `Fragments` and more).
...
x6 = CategoricalInput(
    ...
    # one value per category, per descriptor
    descriptors=Descriptors(
        columns={
            "viscosity (mPa s)": [1.0, 0.59, 1.2],
            "density (kg/m3)": [997, 792, 789],
        },
    ),
)
```
