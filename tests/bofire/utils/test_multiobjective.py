...
of5 = ContinuousOutput(
    desirability_function = MaximizeSigmoidObjective(w=1, tp=1, steepness=1),
    key = "of5",
)
...
def test_infer_ref_point(domain, experiments,return_masked, expected):
    ...
    assert np.allclose(np.array([ref_point[feat] for feat in domain.output_features.get_keys_by_objective(excludes=None)]),
        np.array([expected[feat] for feat in domain.output_features.get_keys_by_objective(excludes=None)]))
