...
def test_RandomStrategy(
    ...
    strategy = RandomStrategy(domain, use_sobol=use_sobol, reduce=reduce)
...
def test_RandomStrategyConstraints(domain,candidate_count, reduce, unit_scaled):
    strategy = RandomStrategy(domain, use_sobol=False, reduce=reduce, unit_scaled=unit_scaled)
    candidates = strategy.ask(
        candidate_count=candidate_count
    )
    assert len(candidates) == candidate_count
...
def test_RandomStrategyReduce():
    ...
    rec = RandomStrategy(domain, reduce = False)
    data, _ = rec.ask(4)
    ...
    rec = RandomStrategy(domain, reduce = True)
    rec.tell(data)
