...
def test_invalid_qehvi_init_domain(domain, ref_point):
    with pytest.raises(ValueError):
        BoTorchQehviStrategy(domain, ref_point=ref_point)
...
def test_qehvi_get_adjusted_refpoint(domain, ref_point, experiments, expected):
    strategy = BoTorchQehviStrategy(domain, ref_point=ref_point)
...
def test_qehvi(strategy, use_ref_point, num_test_candidates):
    ...
    # init strategy
    my_strategy = strategy(benchmark.domain, ref_point=benchmark.ref_point if use_ref_point else None)
