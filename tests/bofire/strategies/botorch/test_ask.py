...
def test_ask_multi_objective(cls, spec, use_ref_point, categorical, descriptor, candidate_count):

    # generate data
    benchmark = DTLZ2(dim=6) #TODO: expand benchmark also towards categorical features?
    random_strategy = RandomStrategy(benchmark.domain)
    experiments = benchmark.run_candidate_experiments(random_strategy.ask(candidate_count=10)[0])
    
    # set up of the strategy
    strategy = cls(**spec, domain=benchmark.domain, ref_point=benchmark.ref_point if use_ref_point else None)
    strategy.tell(experiments)
