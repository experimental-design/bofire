...
def test_LookUpTable_benchmark(cls_benchmark: LookUpTableBenchmark):
    ...
    benchmark_function = cls_benchmark(domain=domain, LookUpTable=look_up)
    # Test, if domain gets set up correctly
    assert benchmark_function.domain == domain
    # Test, if lookup table gets set up correctly
    pd.testing.assert_frame_equal(benchmark_function.LookUpTable, look_up)
    ...
    domain = Domain(inputs=inputs, outputs=outputs)
    benchmark_function = cls_benchmark(domain=domain, LookUpTable=look_up)
    sampled = look_up[:2].copy()
    benchmark_function._f(sampled, return_complete=True)
    # Test, if output function raises error
    sampled.loc[1, "x2"] = "a"
    with pytest.raises(
        ValueError, match=r"Input combination \d+ not found in Look up table"
    ):
        benchmark_function._f(sampled, return_complete=True)
