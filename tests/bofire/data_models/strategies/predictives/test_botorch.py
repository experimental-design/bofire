...
from bofire.data_models.strategies.api import LSRBOConfig, SoboStrategy
...
def test_LSRBOConfig(gamma, acqf_local, acqf_global, expected):
    assert (
        LSRBOConfig(gamma=gamma).is_local_step(
            acqf_local=acqf_local, acqf_global=acqf_global
        )
        == expected
    )
