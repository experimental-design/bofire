...
def test_constraints_jacobian(constraints, num_candidates):
    ...
    for i, c in enumerate(constraints):
        ...
        if isinstance(c, NonlinearConstraint):
            res = candidates.eval(c.jacobian_expression, engine="python")
...
def test_linear_equality_constraint_to_description():
...
def test_linear_inequality_constraint_to_description():
...
def test_nchoosek_constraint_to_description():
...
def test_nchoosek_constraint_to_description_none_valid():
    c = NChooseKConstraint(
        features=["x1", "x2"],
        min_count=1,
        max_count=2,
...
def test_product_equality_constraint_to_description():
    from bofire.data_models.constraints.api import ProductEqualityConstraint
...
def test_product_inequality_constraint_to_description():
    from bofire.data_models.constraints.api import ProductInequalityConstraint
