...
def chem_domain_simple(request) -> tuple[domain_api.Domain, pd.DataFrame, pd.DataFrame]:
    ...
    mols2 = ["NCC(=O)O", "CC(C(=O)O)N", "CC(C)C(C(=O)O)N", "CC(C(C(=O)O)N)O", "C1=CC=C(C=C1)CC(C(=O)O)N",
    ...
    domain = domain_api.Domain(
        inputs=domain_api.Inputs(
            features=[
                features_api.CategoricalMolecularInput(key="molecules", categories=mols1)
            ]
        ),
    ...
    if not multi_input:
        ...
        Y = pd.DataFrame({"output": np.random.uniform(high=4., size=(20,))})
