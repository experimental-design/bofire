...
def chem_domain_simple() -> tuple[domain_api.Domain, pd.DataFrame, pd.DataFrame]:
    domain = domain_api.Domain(
        inputs=domain_api.Inputs(
            features=[features_api.CategoricalMolecularInput(
                key="molecules",
                categories=["C(O)O", "O", "CC"]
            )]
        ),
        outputs=domain_api.Outputs(features=[features_api.ContinuousOutput(key="output")])
    )
    X = pd.DataFrame({"molecules": ["C(O)O", "O", "CC"]})
    Y = pd.DataFrame({"output": [1., 2., 3.]})
    return domain, X, Y


def test_re_init_kwargs_fingerprints(chem_domain_simple: tuple[domain_api.Domain, pd.DataFrame, pd.DataFrame]):
