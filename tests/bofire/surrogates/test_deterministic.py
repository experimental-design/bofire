...
def test_linear_deterministic_surrogate_engineered():
    surrogate_data = LinearDeterministicSurrogate(
        inputs=Inputs(
            features=[
                ContinuousInput(key="a", bounds=(0, 1)),
        ...
        engineered_features=EngineeredFeatures(
            features=[ProductFeature(key="a*a", features=["a", "a"])]
        ),
        outputs=Outputs(features=[ContinuousOutput(key="y")]),
        intercept=2.0,
        coefficients={"b": 3.0, "a": -2.0, "a*a": 3},
    )
    surrogate = surrogates.map(surrogate_data)
    assert surrogate.input_preprocessing_specs == {}
    experiments = pd.DataFrame(data={"a": [1.0, 2.0], "b": [0.5, 4.0]})
    preds = surrogate.predict(experiments)
    assert_frame_equal(
        preds, pd.DataFrame(data={"y_pred": [1.5 + 3, 10.0 + 12], "y_sd": 0.0})
    )
