...
import pytest
import torch
from botorch.models import MixedSingleTaskGP, MultiTaskGP, SingleTaskGP
...
from bofire.benchmarks.api import Himmelblau, MultiTaskHimmelblau
...
def test_MultiTask_input_preprocessing():
    # test that if wrong encoding is used, there is an error
    inputs = Inputs(
    ...
    inputs = Inputs(
        features=[ContinuousInput(key="x", bounds=(-1, 1))]
        + [CategoricalInput(key="task_id", categories=["1", "2"])]
    )
    outputs = Outputs(features=[ContinuousOutput(key="y")])
    with pytest.raises(ValueError):
        data_model = MultiTaskGPSurrogate(
            inputs=inputs,
            outputs=outputs,
            input_preprocessing_specs={"task_id": CategoricalEncodingEnum.ORDINAL},
        )
